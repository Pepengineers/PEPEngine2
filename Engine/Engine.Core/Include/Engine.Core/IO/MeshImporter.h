...
namespace Engine::Core
{
	...
	public:
		...
		[[nodiscard]] static std::shared_ptr<Mesh> ImportSingleMeshAsset(const std::filesystem::path& sourcePath, const MeshImportOptions& options = {});
		...
		[[nodiscard]] static std::shared_ptr<Mesh> ImportMeshSubAsset(const std::filesystem::path& sourcePath, std::uint32_t subAssetIndex, const MeshImportOptions& options = {});
	};
}